...
import type { ApiResponse, Application, ApplicationStatus, PaginationMeta } from "@/lib/types";
...
export const applicationService = {
  ...
  async listAll(filters: ApplicationFilters = {}): Promise<ApplicationListResponse> {
  ...
  async getMy(page = 1, limit = 10): Promise<ApplicationListResponse> {
    const { data } = await api.get<ApiResponse<Application[]>>("/applications/my", {
      params: { page, limit },
    });
    return { data: data.data, meta: data.meta! };
  },
  ...
  async getStatus(id: string): Promise<ApplicationStatus> {
    const { data } = await api.get<ApiResponse<ApplicationStatus>>(`/applications/${id}/status`);
    return data.data;
  },
