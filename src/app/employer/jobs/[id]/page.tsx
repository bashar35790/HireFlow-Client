...
function JobDetail() {
  ...
  return (
    <div className="mx-auto w-full max-w-5xl flex-1 px-4 py-10 sm:px-6">
      ...
      <h2 className="mb-4 text-xl font-bold text-foreground">Applications</h2>
          ...
          {apps.data.map((application) => (
            <Card key={application.id} variant="secondary">
              <Card.Header className="flex-wrap gap-4">
                <div className="flex size-11 shrink-0 items-center justify-center rounded-full bg-foreground/10 text-sm font-bold text-foreground/70">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="text-sm text-foreground/60">
                    {application.user?.email} · Applied{" "}
                    ...
                    <ListBox>
                      {APPLICATION_STATUSES.map((s) => (
                        <ListBox.Item
                          key={s}
                          id={s}
                          textValue={APPLICATION_STATUS_LABEL[s]}
                        >
