...
function EmployerDashboard() {
        ...
        <Card variant="secondary">
          <Card.Content className="gap-4 py-8 text-center">
            ...
            <p className="text-foreground/60">
              Create your company profile first so you can post jobs and review
              applicants.
            </p>
            <div>
  ...
  const publishedCount =
    myJobs?.data.filter((j) => j.status === "PUBLISHED").length ?? 0;
  const draftCount =
    myJobs?.data.filter((j) => j.status === "DRAFT").length ?? 0;
    ...
    <div className="mx-auto w-full max-w-5xl flex-1 px-4 py-10 sm:px-6">
      <div className="mb-8 flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <p className="mt-1 flex items-center gap-2 text-foreground/60">
            {company.location}
            <Chip
              size="sm"
              variant="soft"
      ...
      <div className="mb-8 grid grid-cols-1 gap-4 sm:grid-cols-3">
        <Card variant="secondary">
          <Card.Content className="items-center gap-1 py-6 text-center">
            ...
            <p className="text-sm text-foreground/60">Total jobs</p>
          </Card.Content>
        </Card>
        <Card variant="secondary">
          <Card.Content className="items-center gap-1 py-6 text-center">
        ...
        <Card variant="secondary">
          <Card.Content className="items-center gap-1 py-6 text-center">
            <p className="text-3xl font-bold text-foreground/50">
              {draftCount}
            </p>
      ...
      <div className="mb-4 flex items-center justify-between">
        <h2 className="text-xl font-bold text-foreground">Your jobs</h2>
        <Link
          href="/employer/jobs"
        ...
        <div className="flex flex-col gap-3">
          {myJobs.data.slice(0, 5).map((job) => (
            <Link key={job.id} href={`/employer/jobs/${job.id}`}>
              <Card
                className="transition hover:border-foreground/50 dark:hover:border-foreground/70"
                variant="secondary"
              >
